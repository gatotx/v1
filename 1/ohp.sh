...
Certificate:
    Data:
        ...
        Issuer: C=PH, ST=Manila, L=Quezon, O=-, OU=-, CN=- CA/name=Jokerteam/emailAddress=-
        Validity
            Not Before: Dec  3 12:46:22 2018 GMT
            Not After : Nov 30 12:46:22 2028 GMT
        Subject: C=PH, ST=Jakarta, L=Quezon, O=-, OU=-, CN=Jokerteam/name=Jokerteam/emailAddress=-
        ...
        X509v3 extensions:
            ...
            X509v3 Authority Key Identifier: 
                ...
                DirName:/C=PH/ST=Jakarta/L=Quezon/O=-/OU=-/CN=- CA/name=jokerteam/emailAddress=-
...
echo "<font color=\"green\">Dexter Cellona Banawon (Jokerteam)</font>" > $loc/message

web=$loc/web
mkdir $web 2> /dev/null

MYIP=$(wget -qO- ipv4.icanhazip.com)
cat << ovpnconf > $web/OHP.ovpn
client
dev tun
proto tcp
...
docker run -d --name socksproxyX \
  -v $loc:/conf \
  --net host --cap-add NET_ADMIN jokerteam/smart-bypass:socksproxyX
...
docker run -d --restart always --name badvpn \
 ...
 jokerteam/vpn:badvpn-udpgw --listen-addr 127.0.0.1:7300 --max-clients 1000 --max-connections-for-client 10

echo "Adding menu 'jokerteam'."
bin=/usr/local/bin
cat << 'menu' > $bin/jokerteam
#!/bin/bash
if [ "$(id -u)" -ne 0 ]; then
...
cat << sysctl > /etc/sysctl.d/jokerteam.conf
